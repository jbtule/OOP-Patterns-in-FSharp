...
type Student () =
    ...
    member val LastName = defaultof<string> with get
    member val FirstMidName = defaultof<string> with get,set
    member val EnrollmentDate = defaultof<DateTime> with get,set

    member val Enrollments = defaultof<Enrollemnt ICollection> with get,set
...
type Enrollemnt () =
    ...
    member val Garde = defaultof<Grade Nullable> with get,set
